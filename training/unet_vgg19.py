...
def vgg19_unet(input_shape, classes, dropout):
    """ Input """
    ...
    """ Encoder (feature extraction from VGG19 layers) """
    s1 = vgg19.get_layer("block1_conv2").output  # First block output
    s2 = vgg19.get_layer("block2_conv2").output  # Second block output
    s3 = vgg19.get_layer("block3_conv4").output  # Third block output
    s4 = vgg19.get_layer("block4_conv4").output  # Fourth block output

    # Fifth block (bottom of the U-Net)
...
inp_size = (224, 224, 3)
model = vgg19_unet(inp_size, classes=2, dropout=0.3)
...
# Assuming train_images, train_masks, val_images, and val_masks are already defined
# Train the model
...
# Save the model
model.save('h5_models/segmentasi_gigi_unet_vgg19_augment.h5')
